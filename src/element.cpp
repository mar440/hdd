...
void Element2D::assembly_elasticity(MatrixXd& K, VectorXd &f, vtkCell* cell, double E_mu[]){
	...
	GaussPoints(m_numberOfGaussPoints, r_GP, r_GP, r_GP, weights);
	...
	for (int i = 0; i < m_numberOfGaussPoints; i++){
		for (int j = 0; j < m_numberOfGaussPoints; j++){		 
			...
			x_rs = 0; y_rs = 0; dxdr = 0; dxds = 0; dydr = 0;   dyds = 0; 
...
void QUADRATIC_QUAD::shapeFun(VectorXd &N, VectorXd &dNr, VectorXd &dNs, VectorXd &dummyV, double r, double s, double dummyD){
